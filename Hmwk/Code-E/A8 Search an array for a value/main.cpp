 ...
 * Created on Sept 21, 2025 AND 6:17 PM
...
int  srch1(const char sntnce[],const char pattern[],int index) { //Search for 1 occurrence
    for(int i=index;i<=strlen(sntnce)-strlen(pattern);i++) {  //find first match of pattern in sentence
        int j=0;
        while(j<strlen(pattern)&&sntnce[i+j]==pattern[j]){ //check if match
            j++;
        }
        if (j==strlen(pattern))  //found 
        return i;
    }
    return -1; //not found
}
...
void srchAll(const char sntnce[],const char pattern[],int match[]) { //Search for all occurrences
    int found=0;
    int index=0;
    int pos=srch1(sntnce,pattern,index);
    while(pos != -1) {   //find all matches and store
        match[found++]=pos;
        index=pos + 1;
        pos=srch1(sntnce,pattern,index);
    }
    match[found]=-1;
}
...
void print(const int a[]) {  //Print the array of indexes where the pattern found
    int i;
    for (i=0;a[i]!=-1;i++ ) {   //show match positions
        cout<<a[i]<<endl;
    }
    if (i==0)
    cout<<"None"<<endl;
