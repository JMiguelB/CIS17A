 ...
 * Created on Sept 27th, 2025, 7:01 PM
 */
...
int *mode(const int *array,int arySize){
    ...
    for(int i=1;i<arySize;i++){
        if(ary[i]==ary[i-1]) cnt++;
        else { 
            if(cnt>maxFreq) maxFreq=cnt; cnt=1;
        }
    }
    if(cnt>maxFreq) maxFreq=cnt;  //tail run

    if(maxFreq==1){
        int *modeAry = new int[2];
        modeAry[0] = 0;   // number of modes
        modeAry[1] = 1;   // frequency
        delete []ary;
        return modeAry;
    }
    ...
    for(int i=1;i<arySize;i++){
        if(ary[i]==ary[i-1]) cnt++;
            ...
            if(cnt==maxFreq) nmodes++; 
            cnt=1; 
        }
    }
    if(cnt==maxFreq) nmodes++;    //handle tail run
    ...
    for(int i=1;i<arySize;i++){
        if(ary[i]==ary[i-1])cnt++;
            ...
            if(cnt==maxFreq)modeAry[index++]=ary[i-1];
            cnt=1;
        }
    }
    if(cnt==maxFreq)modeAry[index++]=ary[arySize-1];//Check last run
